...
window.onload = function() {
    // var usernames = ' ';
    // var suer = ['磁暴步兵', '长歌行°', '神秘的群主'];
    // var suber = ['STED', 'KSIE', 'CHGS'];
    // if (usernames != '') {
    //     for (var i = 0; i < suber.length; i + 1) {
    //         if (usernames = suber[i]) {
    //             usernames = suer[i];
    //         }
    //     }
    // } else {
    //     usernames = prompt("请输入:");
    //     for (var i = 0; i < suber.length; i + 1) {
    //         if (usernames = suber[i]) {
    //             usernames = suer[i];
    //         }
    ...
    var video = $('#video1').videoCt({
        ...
        clarity: {
            type: ['360P', ' '], //清晰度
            src: ['depend\\movice\\dianjujinghun.mp4',
                    ''
                ] //链接地址
        },
